...
CUT
        ...
        $form->addText('api_key', array('class' => 'el-wide'))->setLabel('Activecampaign API Key');

        $form->addText('api_user', array('class' => 'el-wide'))->setLabel('Activecampaign Admin Login');
        $form->addPassword('api_password', array('class' => 'el-wide'))->setLabel('Activecampaign Admin Password');
        ...
        $method = self::METHOD_POST)
    {
        ...
        if ($method == self::METHOD_POST) {
            ...
            if($this->plugin->getConfig('debug')) {
                Am_Di::getInstance()
                    ->errorLogTable
                    ->log("[{$corrid}] ACTIVECAMPAIGN POST REQUEST : $url" . 
            ...
            if($this->plugin->getConfig('debug')) {
                Am_Di::getInstance()
                    ->errorLogTable
                    ->log("[{$corrid}] ACTIVECAMPAIGN GET REQUEST : $url");
            }
        }
        ...
        if($this->plugin->getConfig('debug')) {
            Am_Di::getInstance()
                ->errorLogTable
                ->log("[{$corrid}] ACTIVECAMPAIGN RESPONSE : " . 
                    var_export($arr, true));
        }
        ...
        if ($arr['result_code'] != 1) {
            Am_Di::getInstance()
                ->errorLogTable
                ->log("[{$corrid}] Activecampaign API Error - "
