...
CUT
    ...
    public function createWidgetSalesConfigForm()
    {
        ...
        $form->addSortableMagicSelect('display_left', null, array('options' => $this->getWidgetSalesInfoTypes()))
            ->setLabel(___('Display (Left Column)'))
            ->setValue($this->getWidgetSalesDefaultInfoTypes('left'));

        $form->addSortableMagicSelect('display_right', null, array('options' => $this->getWidgetSalesInfoTypes()))
    ...
    {
        return $side == 'left' ?
            array('payments', 'refunds', 'aff-comm') :
            array('signups', 'cancels', 'rebills-next');
    }
    ...
    function getWarnings()
    {
        ...
        $ext = array('pdo', 'pdo_mysql', 'openssl', 'mbstring', 'iconv', 'xml', 'xmlwriter', 'xmlreader');
        foreach ($ext as $e) {
