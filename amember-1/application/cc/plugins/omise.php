...
class Am_Paysystem_Omise extends Am_Paysystem_CreditCard
{
    ...
    {
        $form->addText('pkey', array('class' => 'am-el-wide'))
            ->setLabel('Public Key')
            ->addRule('required');
        $form->addSecretText('skey', array('class' => 'am-el-wide'))
            ->setLabel('Secret Key')
            ->addRule('required');
    }
    ...
    jQuery("#cc").submit(function(event) {
        ...
        Omise.createToken("card", card, function(statusCode, response) {
            if (response.hasOwnProperty('responseJSON') && response.responseJSON.object == 'error') {
                  ...
                  var err = jQuery('<span class="am-error"></span>').text(response.responseJSON.message);
                  el.after(err);
