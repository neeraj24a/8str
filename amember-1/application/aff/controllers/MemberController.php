...
class Aff_MemberController extends Am_Mvc_Controller
{
        ...
        if ($this->getDi()->config->get('aff.affiliate_can_view_details') && $detailDate = $this->getFiltered('detailDate')) {
            ...
            foreach ($this->getDi()->affCommissionTable->fetchByDate($detailDate, $this->user->user_id) as $c) {
                ...
                $user_title = trim(sprintf('%s%s',
                    ($vn ? $this->escape($u->name_f . '&nbsp;' . $u->name_l) : ''),
                    ($ve ? sprintf($ep, $this->escape($u->email)) : '')));
                $s = sprintf('%s%s &ndash; <strong>%s</strong>%s',
                    ($user_title ? "{$user_title} " : ''),
                    ($user_title ? "(" . ___($product->title) . ")" : ___($product->title)),
    ...
    public function getKeywordsReport($uid, $from=null, $to=null)
    {
        ...
        $db->query(<<<CUT
            INSERT INTO ?_aff_keywords_tmp (keyword_id, clicks_count)
                SELECT keyword_id, COUNT(*) FROM ?_aff_click
                    ...
                    GROUP BY keyword_id
CUT
            , $uid, $from_t ?: DBSIMPLE_SKIP, $to_t ?: DBSIMPLE_SKIP);

        //clicks unique
        $db->query(<<<CUT
            INSERT INTO ?_aff_keywords_tmp (keyword_id, clicks_unique_count)
                SELECT keyword_id, COUNT(DISTINCT remote_addr) FROM ?_aff_click
                    WHERE keyword_id IS NOT NULL
                        AND aff_id = ?
                        {AND time >= ?}
                        {AND time <= ?}
        ...
        $db->query(<<<CUT
            INSERT INTO ?_aff_keywords_tmp (keyword_id, leads_count)
                SELECT keyword_id, cnt FROM
                    (SELECT keyword_id, COUNT(*) as cnt from ?_aff_lead
                        WHERE keyword_id IS NOT NULL
                            AND aff_id = ?
                            {AND time >= ?}
                            {AND time <= ?}
...
CUT
            , $uid, $from ?: DBSIMPLE_SKIP, $to ?: DBSIMPLE_SKIP);
