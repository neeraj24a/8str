...
CUT;
    ...
    protected function _initSetupForm(Am_Form_Setup $form)
    {
        ...
        $form->addText('legal_name', array('class' => 'am-el-wide'))
            ...
            ->addRule('required');
        $form->addTextarea('legal_address', array('class' => 'am-el-wide', 'rows' => 5))
...
(function(){
    jQuery('#auddis').change(function(){
        jQuery('#auddis-login').closest('.am-row').toggle(this.checked);
    }).change();
...
CUT
    ...
    function getPaymentDate($lead_time = false)
    {
        ...
        return date('Y-m-' . sprintf("%02d", $days[0]), strtotime('first day of next month', $n));
    }
...
%s', $this->escape($address['bank_name']), $this->escape($address['branch']), $street, $this->escape($address['town']), $this->escape($address['county']), $this->escape($address['postcode'])))
        ...
        $form->addStatic()
            ...
            ->setLabel('Reference');
        $form->addStatic(null, array('class' => 'am-no-label'))
            ->setContent(<<<CUT
...
CUT
        ...
        $g = $form->addGroup(null, array('class' => 'am-no-label'));
        $g->setSeparator('&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;');
...
class Am_Form_Echeck_SmartDebit extends Am_Form_Echeck
{
    ...
    public function init()
    {
        ...
        $fieldSetBank->addStatic(null, array('class' => 'am-no-label'))
