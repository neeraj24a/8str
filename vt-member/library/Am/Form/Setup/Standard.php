...
CUT
    ...
    {
        //we will enable it later if there is at least one CC plugin
        $modules = $after->get('modules', array());
        array_remove_value($modules, 'cc');
        $after->set('modules', $modules);
...
class Am_Form_Setup_PersonalData extends Am_Form_Setup
{
    ...
    function initElements()
    {
        $this->addAdvCheckbox('enable-account-delete', ['id'=>'enable-account-delete'])
            ->setLabel(___("Enable 'Delete Personal Data' link\n"
        ...
        $this->addHTML()->setHTML(
            ___('Use %sAgreement  Editor%s to create "End User Agreement",  "Privacy Plolicy" or "Terms of Use"',
...
jQuery(function(){
    ...
    jQuery('#account-removal-method').on('change', function(){
            jQuery("#keep-fields").closest('.row').toggle(jQuery(this).val() == 'anonymize' || jQuery(this).val() == 'delete-request');
            jQuery("#keep-payout").closest('.row').toggle(jQuery(this).val() == 'anonymize' || jQuery(this).val() == 'delete-request');
    }).trigger('change');
