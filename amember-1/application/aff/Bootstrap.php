...
class Bootstrap_Aff extends Am_Module
{
    ...
    {
        $e->addReturn(new Am_AdminDashboardWidget('aff-top-affiliate', ___('Top Affiliate'), array($this, 'renderWidgetTopAffiliate'), Am_AdminDashboardWidget::TARGET_ANY, array($this, 'createWidgetTopAffiliateConfigForm'), self::ADMIN_PERM_ID));

        $e->addReturn(new Am_AdminDashboardWidget('aff-last-user',  ___('Recent Affiliates'), array($this, 'renderWidgetLastUser'), Am_AdminDashboardWidget::TARGET_ANY, array($this, 'createWidgetNumForm'), self::ADMIN_PERM_ID));
        $e->addReturn(new Am_AdminDashboardWidget('aff-last-comm',  ___('Recent Affiliate Commissions'), array($this, 'renderWidgetLastComm'), Am_AdminDashboardWidget::TARGET_ANY, array($this, 'createWidgetNumForm'), self::ADMIN_PERM_ID));
        $e->addReturn(new Am_AdminDashboardWidget('aff-last-click', ___('Recent Affiliate Clicks'), array($this, 'renderWidgetLastClick'), Am_AdminDashboardWidget::TARGET_ANY, array($this, 'createWidgetNumForm'), self::ADMIN_PERM_ID));
...
jQuery("#' . $el->getId() . '").change(function()
{
    ...
    jQuery("option", jQuery(this)).each(function(){
        ...
        if(option == selected){
            jQuery("input[name^=aff_"+option+"_],textarea[name^=aff_"+option+"_],select[name^=aff_"+option+"_]").closest(".am-row").show();
        }else{
            jQuery("input[name^=aff_"+option+"_],textarea[name^=aff_"+option+"_],select[name^=aff_"+option+"_]").closest(".am-row").hide();
        }
    });
}).change();
});
...
CUT
    ...
    public function onGridUserInitForm(Am_Event_Grid $event)
    {
        ...
        $fieldSet = $event->getGrid()->getForm()
            ->addAdvFieldset(null, array('id' => 'affiliate'))
            ->setLabel(___('Affiliate Program'));
...
CUT
    ...
    public function onGridUserInitGrid(Am_Event_Grid $event)
    {
        $event->getGrid()->actionAdd(new Am_Grid_Action_Aff_Detach);
    }
...
//        }
    ...
    function onUserAfterUpdate(Am_Event_UserAfterUpdate $e)
    {
        ...
        elseif($e->getUser()->is_affiliate && !$e->getOldUser()->is_affiliate && ($this->getConfig('signup_type') == 2))
            $this->sendAffRegistrationEmail($e->getUser());
