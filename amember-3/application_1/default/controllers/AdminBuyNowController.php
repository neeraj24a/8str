...
CUT;
    ...
    public function createForm()
    {
        ...
        $form = $mainForm->addFieldset()->setLabel(___('BuyNow Button'));

        $form->addText('title', 'class=el-wide')->setLabel(___("Title"))
            ->addRule('required');
        $form->addText('comment', 'class=el-wide')->setLabel(___("Comment"));

        $sel = $form->addSelect('billing_plan_id')->setLabel(___("Product"));
        $sel->addRule('required');
        $sel->loadOptions($this->getDi()->billingPlanTable->getOptions());

        $form->addText('coupon')
            ->setLabel(___('Coupon'));

        $sel = $form->addSelect('saved_form_id')
        ...
        $opt = array(''=>'[default form]') + $this->getDi()->savedFormTable->getOptions(SavedForm::T_SIGNUP);
        $sel->loadOptions($opt);

        $sel = $form->addSelect('paysys_id')
            ->setLabel(___('Payment System'));
        $sel->addRule('required');
        $sel->loadOptions(array_merge(array('free'=>'Free'), $this->getDi()->paysystemList->getOptionsPublic()));
