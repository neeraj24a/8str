...
jQuery(function(){
    jQuery('#video-player').change(function(){
        jQuery('#video-player-Flowplayer').closest('.am-row').toggle(jQuery(this).val() == 'Flowplayer');
        jQuery('#video-player-JWPlayer').closest('.am-row').toggle(jQuery(this).val() == 'JWPlayer');
    }).change();
})
CUT
            );
...
CUT
        ...
        $g->setSeparator(' ');
        $g->addSelect('plugins.tax', array('size' => 1, 'id'=>'plugins-tax'))
            ->loadOptions(array('' => ___('No Tax')) + $tax_plugins);
...
CUT
        ...
        $this->addSelect('dashboard_page')
            ->setLabel(___("Dashboard Page\n" .
                ...
                'you can create new pages %shere%s', '<strong>', '</strong>',
                '<a class="link" href="' . Am_Di::getInstance()->url('default/admin-content/p/pages') . '">', '</a>'))
            ->loadOptions(array(''=>___('Default Dashboard Page')) +
                Am_Di::getInstance()->db->selectCol("SELECT page_id AS ?, title FROM ?_page", DBSIMPLE_ARRAY_KEY));
...
class Am_Form_Setup_Plugins extends Am_Form_Setup
{
    ...
    function initElements()
    {
        ...
        if (!Am_Di::getInstance()->modules->isEnabled('newsletter') &&
            file_exists(AM_APPLICATION_PATH . '/newsletter/plugins')) {

            $plugins = self::getPluginsList(array(AM_APPLICATION_PATH . '/newsletter/plugins'));
            $this->addMagicSelect('plugins.newsletter', array('class' => 'magicselect am-combobox-fixed'))
                ->setLabel(___('Newsletters Plugins'))
                ->loadOptions($plugins);
        }
...
CUT
    ...
    public function beforeSaveConfig(Am_Config $before, Am_Config $after)
    {
        ...
        //explicitly disabled module - disable all its plugins
        if (Am_Di::getInstance()->modules->isEnabled('newsletter') &&
            false === array_search('newsletter', $modules)) {

            $after->set('plugins.newsletter', array());
        }

        if ($after->get('plugins.newsletter') &&
            !Am_Di::getInstance()->modules->isEnabled('newsletter')) {

            $modules[] = 'newsletter';
            $all_enabled = array_merge($all_enabled, $after->get('plugins.newsletter'));
        }
...
class Am_Form_Setup_Email extends Am_Form_Setup
{
    ...
    function initElements()
    {
        ...
        $this->addText('technical_email', array('class' => 'am-el-wide'))
        ...
        $fs->addText('smtp_host', array('class' => 'am-el-wide'), array('help-id' => '#SMTP_Mail_Settings'))
        ...
        $fs->addText('ses_id', array('class' => 'am-el-wide'))
            ->setLabel(___('Amazon SES Access Id'));
        $fs->addPassword('ses_key', array('class' => 'am-el-wide'))
        ...
        $fs->addPassword('sendgrid3_key', array('class' => 'am-el-wide'))
            ->setLabel(___('SendGrid API Key'))
            ->addRule('required', ___('This field is required'), null, HTML_QuickForm2_Rule::CLIENT);

        $fs->addText('campaignmonitor_apikey', array('class' => 'am-el-wide'))
            ->setLabel(___('Campaignmonitor API Key'))
            ->addRule('required', ___('This field is required'), null, HTML_QuickForm2_Rule::CLIENT);

        $fs->addText('campaignmonitor_clientid', array('class' => 'am-el-wide'))
            ->setLabel(___('Client API ID'))
            ->addRule('required', ___('This field is required'), null, HTML_QuickForm2_Rule::CLIENT);

        $fs->addText('mailjet_apikey_public', array('class' => 'am-el-wide'))
            ->setLabel(___('Mail Jet API Public Key'))
            ->addRule('required', ___('This field is required'), null, HTML_QuickForm2_Rule::CLIENT);

        $fs->addPassword('mailjet_apikey_private', array('class' => 'am-el-wide'))
            ->setLabel(___('Mail Jet API Private Key'))
            ->addRule('required', ___('This field is required'), null, HTML_QuickForm2_Rule::CLIENT);

        $fs->addPassword('postmark_token', array('class' => 'am-el-wide'))
...
CUT
        ...
        $fs->addElement('email_checkbox', 'send_refund_mail', null, array('help-id' => '#Email_Messages_Configuration'))
            ->setLabel(___("E-Mail Refund Receipt to User\n".
                'every time payment is refunded'));

        $fs->addElement('email_checkbox', 'send_refund_admin', null, array('help-id' => '#Email_Messages_Configuration'))
...
CUT
        ...
        $fs->addTextarea('unsubscribe_html', array('class' => 'am-el-wide', 'rows'=>6),  array('help-id' => '#Miscellaneous_Email_Settings'))
        ...
        $fs->addTextarea('unsubscribe_txt', array('class' => 'am-el-wide', 'rows'=>6),  array('help-id' => '#Miscellaneous_Email_Settings'))
        ...
        $fs->addText('copy_admin_email', array('class' => 'am-el-wide'), array('help-id' => '#Miscellaneous_Email_Settings'))
...
CUT
    ...
    function initElements()
    {
        ...
        $g->addAdvCheckbox('pdf_refund_sent_user', null, array('content' => ___('Attach invoice file (.pdf) to Refund Receipt to User')));
        $g->addAdvCheckbox('pdf_refund_sent_admin', null, array('content' => ___('Attach invoice file (.pdf) to Refund Receipt to Admin')));

        $this->addText('invoice_filename', array('size'=>30, 'class' => 'am-el-wide'))
...
CUT
        ...
        $this->addText('multi_title', array('class' => 'am-el-wide'), array('help-id' => '#Configuring_Advanced_Settings'))
...
CUT;
        ...
        $fs->addTextarea('manually_approve_note', array('rows' => 8, 'class' => 'am-el-wide'))
...
CUT
        ...
        $fs->addText('login_meta_title', array('class' => 'am-el-wide'))
            ->setLabel(___("Login Page Title\nmeta data (used by search engines)"));
        $fs->addText('login_meta_keywords', array('class' => 'am-el-wide'))
            ->setLabel(___("Login Page Keywords\nmeta data (used by search engines)"));
        $fs->addText('login_meta_description', array('class' => 'am-el-wide'))
...
class Am_Form_Setup_Recaptcha extends Am_Form_Setup
{
    ...
    function initElements()
    {
        $this->addText("recaptcha-public-key", array('class'=>'am-el-wide'), array('help-id' => 'Setup/ReCaptcha'))
        ...
        $this->addText("recaptcha-private-key", array('class'=>'am-el-wide'), array('help-id' => 'Setup/ReCaptcha'))
