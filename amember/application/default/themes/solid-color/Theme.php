...
CUT
        ...
        $form->addSelect('font_family')
                ->setLabel(___('Font Family'))
                ->loadOptions($this->getFontOptions());

        $gr = $form->addGroup()
                ->setLabel(___('Font Size'));
        $gr->setSeparator(' ');
    ...
    function addElementColor($form, $name, $label, $id = null) {
        ...
        foreach (array('#f1f5f9', '#dee7ec', '#ffebcd', '#ff8a80', '#ea80fc',
                ...
                $gr->addHtml()->setHtml("<div class='color-pick' style='background:{$color}' data-color='$color'></div>");
        }
    }
