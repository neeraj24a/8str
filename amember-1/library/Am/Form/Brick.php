...
class Am_Form_Brick_HTML extends Am_Form_Brick
{
    ...
    public function initConfigForm(Am_Form $form)
    {
        ...
        $form->addHtmlEditor('html', array('rows' => 15, 'class' => 'html-editor'), array('dontInitMce' => true))
            ...
            ->setMceOptions(array('placeholder_items' => $this->getUserTagOptions()));

        $form->addText('label', array('class' => 'am-el-wide', 'rel' => 'position-default'))->setLabel(___('Label'));
        ...
        if ($fields) {
            ...
            $current_val = $this->getConfig('cond_field_val');
            $gr = $form->addGroup();
            $gr->setLabel(___('Conditional'));
            $gr->setSeparator(' ');
            $gr->addHtml()->setHtml('<div class="html-conditional-group">');
...
jQuery(function(){
    if (!window.brick_html_init) {
        ...
        jQuery(document).on('change', '.brick-html-position', function(){
            jQuery(this).
                closest('form').
                find('[rel=position-default], [name=cond_enabled]').
                closest('.am-row').
                toggle(jQuery(this).val() == '');
        });
        jQuery('.brick-html-position').change();

        jQuery(document).on('change', '.html-conditional-group [name=cond_enabled]', function(){
            jQuery(this).nextAll().toggle(this.checked);
        });
        jQuery('.html-conditional-group [name=cond_enabled]').change();

        jQuery(document).on('change', '.html-conditional-group [name=cond_field]', function(){
            var context = jQuery(this).closest('.html-conditional-group');
            var current_val = jQuery(this).data('current_val');
            jQuery("[name=cond_field_val]", context).empty();
            var cOpt = jQuery(this).data('opt')[jQuery(this).val()];
            for (var k in cOpt) {
                jQuery("[name=cond_field_val]", context).append(
                    jQuery('<option>').text(cOpt[k]).attr('value', k)
                );
            }
            if (current_val != undefined) {
                jQuery("[name=cond_field_val]", context).val(current_val);
                jQuery(this).data('current_val', undefined);
            } else {
                jQuery("[name=cond_field_val]", context).val(jQuery("[name=cond_field_val] option:first", context).attr('value'));
            }
        });
        jQuery('.html-conditional-group [name=cond_field]').change();
    }
});
CUT
            );
...
CUT
            ...
            default:
                ...
                $this->addConditionalIfNecessary($form, 'row-html' . self::$counter . '-0');
        }
    }

    function addConditionalIfNecessary($form, $id)
    {
...
jQuery('select[name=$c_field_name],\
    ...
    val = val || 0;
    jQuery('#{$id}').toggle(val $c_cond $c_field_val);
}).change();
CUT
                    );
        }
...
class Am_Form_Brick_JavaScript extends Am_Form_Brick
{
    public function initConfigForm(Am_Form $form)
    {
        $form->addTextarea('code', array('rows' => 15, 'class' => 'am-el-wide'))
    ...
    public function insertBrick(HTML_QuickForm2_Container $form)
    {
        ...
        $email->addRule('callback2', '--wrong email--', array($this, 'check'))
            ->addRule('remote', '--wrong email--', array(
                'url' => Am_Di::getInstance()->url('ajax/check-uniq-email', array('_url'=>$redirect), false)
            ));
...
CUT
    ...
    public function insertBrick(HTML_QuickForm2_Container $form)
    {
        ...
        if (!$this->getConfig('do_not_confirm')) {
            $pass0 = $form->addPassword('_pass', array('size' => 30, 'autocomplete'=>'off', 'maxlength' => Am_Di::getInstance()->config->get('pass_max_length', 64)))
...
CUT
    ...
    public function insertBrick(HTML_QuickForm2_Container $form)
    {
        ...
        $form->addHtml()
            ...
            ->setHtml(<<<CUT
<a href="javascript:;" class="local-link am-change-pass-toggle" onclick="jQuery(this).closest('.am-row').hide(); jQuery('.am-change-pass').closest('.am-row').show()">$change</a>
<script type="text/javascript">
  jQuery(function(){
    jQuery('.am-change-pass').closest('.am-row').toggle(jQuery('[name=pass]').val() != '');
    jQuery('.am-change-pass-toggle').closest('.am-row').toggle(jQuery('[name=pass]').val() == '');
  });
</script>
CUT
            );
...
class Am_Form_Brick_Address extends Am_Form_Brick
{
    ...
    public function insertBrick(HTML_QuickForm2_Container $form)
    {
        ...
        foreach ($this->getConfig('fields', array()) as $f => $required) {
            switch ($f) {
                case 'street' :
                    $street = $fieldSet->addText('street', array('class' => 'am-el-wide'))->setLabel($this->___('Street'));
                    if ($required)
                        $street->addRule('required', ___('Please enter %s', $this->___('Street')));
                    break;
                case 'street2' :
                    $street = $fieldSet->addText('street2', array('class' => 'am-el-wide'))->setLabel($this->___('Street (Second Line)'));
                    if ($required)
                        $street->addRule('required', ___('Please enter %s', $this->___('Street (Second Line)')));
                    break;
                case 'city' :
                    $city = $fieldSet->addText('city', array('class' => 'am-el-wide'))->setLabel($this->___('City'));
                    if ($required)
...
EOF;
    ...
    public function initConfigForm(Am_Form $form)
    {
        ...
        $groups = $form->addMagicSelect('groups', array('data-type' => self::DISPLAY_CATEGORY,))
        ...
        $products = $form->addSortableMagicSelect('products', array('data-type' => self::DISPLAY_PRODUCT,))
...
jQuery('select[name=$c_field_name],\
    ...
    val = val || 0;
    jQuery('[name=$name]').closest('.am-row').toggle(val $c_cond $c_field_val);
    jQuery('[name="{$name}[]"]').not('[type=hidden]').closest('.am-row').toggle(val $c_cond $c_field_val);
    jQuery('[data-name="{$name}[]"]').closest('.am-row').toggle(val $c_cond $c_field_val);
}).change();
CUT
                    );
        }
...
CUT
        ...
        if ($fields) {
            ...
            $gr->addScript()
                ->setScript(<<<CUT
jQuery(function(){
    jQuery("#$id [name=cond_enabled]").change(function(){
        jQuery(this).nextAll().toggle(this.checked);
    }).change();
    var current_val = $current_val;
    var opt = $allOp;
    jQuery("#$id [name=cond_field]").change(function(){
        jQuery("#$id [name=cond_field_val]").empty();
        var cOpt = opt[jQuery(this).val()];
        for (var k in cOpt) {
            jQuery("#$id [name=cond_field_val]").append(
                jQuery('<option>').text(cOpt[k]).attr('value', k)
            );
        }
        if (current_val != undefined) {
            jQuery("#$id [name=cond_field_val]").val(current_val);
            current_val = undefined;
        } else {
            jQuery("#$id [name=cond_field_val]").val(jQuery("#$id [name=cond_field_val] option:first").attr('value'));
        }
    }).change();
});
CUT
...
class Am_Form_Brick_Agreement extends Am_Form_Brick
{
    ...
    function insertBrick(HTML_QuickForm2_Container $form)
    {
        ...
        foreach ($agreements as $agreement)
        {
            ...
            if (!$this->getConfig('do_not_show_agreement_text'))
            {
                ...
                {
                    $agr = $conteiner->addStatic('_agreement', array('class' => 'am-no-label'));
    ...
    function initConfigForm(Am_Form $form)
    {
        ...
        if (!Am_Di::getInstance()->agreementTable->getTypeOptions()) {
            $gr->addTextarea("_agreement_text", array('rows' => 20, 'class' => 'am-el-wide'));
...
class Am_Form_Brick_RandomQuestions extends Am_Form_Brick
{
    ...
    public function initConfigForm(Am_Form $form)
    {
        $form->addTextarea('questions', array('rows' => 10, 'class'=>'am-el-wide'))
    ...
    public function insertBrick(HTML_QuickForm2_Container $form)
    {
        ...
        switch ($this->getConfig('position', 'above')) {
            ...
            case 'below' :
                ...
                break;
            case 'brick' :
                $form->addHtml(null, array('class'=>'am-row-wide'))
...
jQuery(document).ready(function($) {
    ...
    jQuery(".am-signup-form").on('change', ":checkbox[name^='product_id'], select[name^='product_id'], :radio[name^='product_id'], input[type=text][name^='product_id'], input[type=hidden][name^='product_id'], [name=paysys_id], [name=coupon], :checkbox[name^='productOption'], select[name^='productOption'], :radio[name^='productOption'], input[type=text][name^='productOption'], textarea[name^='productOption'], select[name='country'], input[name='tax_id'], select[name='state'], input[name='zip'], input[name='giftVoucherCode']", function(){
