    ...
    public function prepare(Am_View $view)
    {
        ...
        foreach ($view->payments as $payment) {
            ...
            if ($view->di->config->get('send_pdf_invoice') && $view->di->config->get('pdf_invoice_link')) {
                $payment->_invoice_url = $view->url(array('member/get-invoice/id/%s', $view->obfuscate($payment->pk())), false);
            }
        }
    }
    public function getTitle()
...
class Am_Widget_DetailedSubscriptions extends Am_Widget
{
    ...
    public function prepare(Am_View $view)
    {
        ...
        foreach ($view->activeInvoices as $invoice) {
            ...
            if (empty($_) &&
                in_array($invoice->getStatus(), array(Invoice::PAID, Invoice::RECURRING_ACTIVE, Invoice::RECURRING_CANCELLED))) {
