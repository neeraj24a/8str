...
class Am_Form_Renderer extends HTML_QuickForm2_Renderer_Default
{
    public function __construct()
    {
        ...
        $cp = defined('HP_ROOT_DIR') ? 'am_' : '';
        $this->setTemplateForClass($cp.'html_quickform2_element', '<div class="'.$rowClass.'" id="row-{id}"><qf:label><div class="'.$elTitleClass.'"><label for="{id}"><qf:required><span class="required">* </span></qf:required>{label}</label><qf:label_2><div class="comment">{label_2}</div></qf:label_2></div></qf:label><div class="'.$elClass.'<qf:error> error</qf:error>">{element}<qf:error><span class="am-error">{error}</span></qf:error></div></div>'."\n");
        $this->setTemplateForClass($cp.'html_quickform2_container_group', '<div class="'.$rowClass.'" id="row-{id}"><qf:label><div class="'.$elTitleClass.'"><label><qf:required><span class="required">* </span></qf:required>{label}</label><qf:label_2><div class="comment">{label_2}</div></qf:label_2></div></qf:label><div class="'.$elClass.' group<qf:error> error</qf:error>">{content}<qf:error><span class="am-error">{error}</span></qf:error></div></div>'."\n");
        $this->setTemplateForClass($cp.'html_quickform2', '<div class="am-form">{errors}<form{attributes}>{content}{hidden}</form><qf:reqnote><div class="reqnote">{reqnote}</div></qf:reqnote></div>'."\n");
        $this->setTemplateForClass($cp.'html_quickform2_container_fieldset', '<fieldset{attributes}><qf:label><legend id="{id}-legend">{label}</legend></qf:label><div class="fieldset">{content}</div></fieldset>'."\n");
        ...
        $this->setElementTemplateForGroupClass('am_form_container_prefixfieldset', $cp.'html_quickform2_element', '<div class="'.$rowClass.'" id="row-{id}"><div class="'.$elTitleClass.'"><label for="{id}"><qf:required><span class="required">* </span></qf:required>{label}</label><qf:label_2><div class="comment">{label_2}</div></qf:label_2></div><div class="'.$elClass.'<qf:error> error</qf:error>">{element}<qf:error><span class="am-error">{error}</span></qf:error></div></div>'."\n");
    }
    ...
    public function findTemplate(HTML_QuickForm2_Node $element, $default = null)
    {
        ...
        if ($element->hasClass('am-no-label')) {
            $ret = str_replace('class="am-row', 'class="am-row am-no-label', $ret);
        }
        if ($element->hasClass('am-row-wide')) {
            $ret = str_replace('class="am-row', 'class="am-row am-row-wide', $ret);
        }
        if ($element->hasClass('am-row-highlight')) {
            $ret = str_replace('class="am-row', 'class="am-row am-row-highlight', $ret);
        }
        if ($element->hasClass('am-row-head')) {
            $ret = str_replace('class="am-row', 'class="am-row am-row-head', $ret);
        }
