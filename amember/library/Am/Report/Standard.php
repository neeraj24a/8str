...
class Am_Report_PaymentCouponBatch extends Am_Report_Date
{
    ...
    public function getQuery()
    {
        ...
        $q = new Am_Query($this->getDi()->invoiceTable);
        $q->leftJoin('?_coupon', 'c', 't.coupon_id=c.coupon_id')
            ->addWhere('status=?', Invoice::PAID)
            ->addWhere('t.coupon_id IS NOT NULL')
            ->addWhere('batch_id=?', $batch_id);

        $q->clearFields();
        $q->addField('ROUND(SUM(first_total/base_currency_multi), 2) AS amt');
        return $q;
    }
