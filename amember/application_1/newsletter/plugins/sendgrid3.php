...
class Am_Newsletter_Plugin_Sendgrid3 extends Am_Newsletter_Plugin
{
    ...
    function _initSetupForm(Am_Form_Setup $form)
    {
        ...
        $el = $form->addPassword('api_key', array('size' => 40))->setLabel('SendGrid API Key' .
...
class Am_Sendgrid_Api3 extends Am_HttpRequest
{
        ...
        function sendRequest($method, $params = array(), $httpMethod = 'POST')
    {
        ...
        if ($this->plugin->getConfig('log'))
            Am_Di::getInstance()->errorLogTable->log(sprintf("%s, method = %s, params=%s, status=%s, resp = %s", $log, $httpMethod, $this->getBody(), $ret->getStatus(), $ret->getBody()));
