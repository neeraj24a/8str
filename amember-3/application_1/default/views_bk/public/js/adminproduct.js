...
$(document).on("click", ".plan-add", function (event) {
    ...
    $("#billing-plan-wrap .plan-add").closest("li").before("<li data-plan_id='"+newId +
...

{
    ...
    var productOptionMethods = {
        ...
        validate: function () { // this=form
            var errors = false;
            $(this).find(".am-product-option:visible").each(function () {
...
(function ($) {
    $.fn.productOptionsEditor = function (action)
    {
        return this.each(function(){
            ...
            $(document.body).append($cssHidePrice);
            $(document.body).append("<style>.option-price-zero { color: gray; }</style>\n\n");
            ...
            function renderOptionOptionItem(key, label, def, prices)
            {
                ...
                for (var i in billingPlans)
                {
                    ...
                    $td.append('<span class=option-price-zero>$</span>')
                    $td.append($fs);
