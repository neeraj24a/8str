    ...
    public function renderMenu(Am_Navigation_Container $container, $options = array()) {
        ...
        foreach ($container as $page)
        {
            ...
            if (!($page->hasChildren() && !$subMenu)) {
                $html .= sprintf('<li class="%s%s"><div class="menu-glyph"%s><div class="menu-glyph-delimeter"><a id="%s" href="%s" class="%s" %s>%s</a></div></div>%s</li>',
...
CUT;
    ...
    protected function renderSubMenu(Am_Navigation_Page $page, $is_closed = true) {
        ...
        foreach ($page as $subPage)
        {
            ...
            $html .= sprintf('<li class="%s"><div class="menu-glyph" %s><a id="%s" href="%s" class="%s" %s>%s</a></div></li>',
                    $active_state,
