 ...
 * @recurring amember
 */
class Am_Paysystem_Stripe extends Am_Paysystem_CreditCard implements Am_Paysystem_TokenPayment
{
    ...
    protected function _initSetupForm(Am_Form_Setup $form)
    {
        ...
        $form->addText('image_url', array('class'=>'am-el-wide', 'rel'=>'hosted-version'))
...
jQuery(function(){
    jQuery('#hosted-version').change(function(){
        jQuery('[rel=hosted-version]').closest('.am-row').toggle(this.checked);
    }).change();
});
CUT
            );
...
CUT;
    ...
    {
        return $invoice->getUser()->data()->get(Am_Paysystem_Stripe::TOKEN);
    }
        ...
        function processTokenPayment(\Invoice $invoice, $token=null)
    {
        ...
        $tr = new Am_Paysystem_Transaction_Stripe($this, $invoice, $invoice->status == Invoice::PENDING);
        $tr->run($result);
        ...
        function saveToken(\Invoice $invoice, $token)
    {
        ...
        $tr = new Am_Paysystem_Transaction_Stripe_CreateCustomer($plugin, $invoice, $source['id']);
        $tr->run($result);
    ...
    jQuery("form#cc-stripe").submit(function(event){
        ...
        Stripe.createToken({
            ...
            if (status == '200')
            {
                ...
                cnt.find("span.am-error").remove();
                el.after("<span class='am-error'><br />"+msg+"</span>");
            }
        });
