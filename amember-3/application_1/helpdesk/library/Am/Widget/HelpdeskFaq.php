...
class Am_Widget_HelpdeskFaq extends Am_Widget
{
    ...
    public function prepare(Am_View $view)
    {
        ...
        if (!empty($view->title)) // display the item
        {
            ...
            $this->bpath = array($this->getDi()->url('helpdesk/faq',null,false) => ___('FAQ'));
            if ($faq->category) {
                $this->bpath[$this->getDi()->url('helpdesk/faq/c/' . urldecode($faq->category),null,false)] = $faq->category;
            }
            $view->faq = $faq;
