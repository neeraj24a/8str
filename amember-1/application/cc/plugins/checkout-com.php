...
class Am_Paysystem_CheckoutCom extends Am_Paysystem_CreditCard implements Am_Paysystem_TokenPayment
{
    ...
    public function _doBill(Invoice $invoice, $doFirst, CcRecord $cc, Am_Paysystem_Result $result)
    {
        ...
        $data = array_merge($data, array(
            'value' => intval(($doFirst ? $invoice->first_total : $invoice->second_total) * 100),
    ...
    protected function _initSetupForm(Am_Form_Setup $form)
    {
        ...
        $form->addSecretText('secret_key', array('class'=>'am-el-wide'))->setLabel('Secret Key')->addRule('required');
        $form->addText('public_key', array('class'=>'am-el-wide'))->setLabel('Publishable Key')->addRule('required');
    ...
    function getPaymentToken($invoice, & $error)
    {
        ...
        $req->setHeader('Authorization', $this->getConfig('secret_key'))
            ...
            ->setBody(json_encode(array(
                'value' => intval($invoice->first_total * 100),
                'currency' => $invoice->currency,
                'chargeMode' => 2,
                'attemptN3D' => 1,
                'trackId' => $invoice->public_id,
...
CUT;
    ...
    {
        return $invoice->getUser()->data()->get(self::CARD_TOKEN);
    }
        ...
        function processTokenPayment(\Invoice $invoice, $token=null)
    {
        ...
        $doFirst = ($invoice->status == Invoice::PENDING);
    ...
    {
        $invoice->getUser()->data()
            ->set(self::CARD_TOKEN, $token)
            ->set(self::CARD_TOKEN_EMAIL, $invoice->getUser()->email)
            ->update();
        return $token;
    }
...
class Am_Controller_CreditCard_CheckoutCom extends Am_Mvc_Controller
{
    ...
    {
        return $this->plugin->createFormHosted($this->invoice, $label, $this);
    }
