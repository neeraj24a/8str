 ...
 * @adult 1
 */
class Am_Paysystem_Epay extends Am_Paysystem_CreditCard implements Am_Paysystem_TokenPayment
{
    ...
    public function _initSetupForm(Am_Form_Setup $form)
    {
        ...
        $form->addAdvCheckbox('hosted')
            ->setLabel(___("Hosted Mode\n"
    ...
    function getPaymentParams(Invoice $invoice, $windowstate=3)
    {
        $p = array();
    ...
    function _process(Invoice $invoice, $request, Am_Paysystem_Result $result)
    {
        if($this->getConfig('hosted')){
            $a = new Am_Paysystem_Action_HtmlTemplate('pay.phtml');
            $a->invoice = $invoice;
            $a->form = $this->getPaymentWindowHtml($invoice);
            $result->setAction($a);
        }else{
            $a = new Am_Paysystem_Action_Redirect(self::FORM_URL);
            $p = $this->getPaymentParams($invoice);
            $a->setParams($p);
            $result->setAction($a);
        }
    }
    ...
    function getEpayError($epay_code, $pbs_code=null)
    {
        ...
        return $xml->getEpayErrorResponse->epayResponseString.(!empty($xml1)?"<br/>".$xml1->getPbsErrorResponse->pbsResponseString:"");
    }
...
CUT;
    ...
    function APIRequest($type='subscription', $function='', $vars=array(), $log=null)
    {
        try{
            ...
            if(!empty($log))
            {
                $log->add(['Refund Request' => $xml], true);
            }
    ...
    function processRefund(InvoicePayment $payment, Am_Paysystem_Result $result, $amount)
    {
        ...
        $log->add(['Refund Response' => $xml->asXML()], true);
        
        if ($xml->creditResponse->creditResult == 'true'){
            $trans = new Am_Paysystem_Transaction_Manual($this);
            $trans->setAmount($amount);
            $trans->setReceiptId($payment->transaction_id.'-epay-refund');
...
CUT;
    ...
    public
        function getPaymentWindowHtml(\Invoice $invoice)
    {
        $this->invoice = $invoice;
        $cancelUrl=$this->getCancelUrl();
        $config = $this->getPaymentParams($invoice,1);
        $config = json_encode($config);
        return <<<CUT
<script src='https://ssl.ditonlinebetalingssystem.dk/integration/ewindow/paymentwindow.js'></script>
<script> 
    jQuery(document).ready(function(){
        paymentwindow = new PaymentWindow({$config});
        paymentwindow.on('close', function(){ window.location.href='{$cancelUrl}'; });
        paymentwindow.open(); 
    });
</script>
CUT;
...
class Am_Paysystem_Transaction_Epay  extends Am_Paysystem_Transaction_Incoming
{
    ...
    {
        if(!$this->request->get('txnid')) 
            return true; // Seems to be CC update;
