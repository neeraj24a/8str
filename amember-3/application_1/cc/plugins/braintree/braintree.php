...
class Am_Paysystem_Braintree extends Am_Paysystem_CreditCard
{
    ...
    protected
        function ccActionValidateSetInvoice(Am_Mvc_Request $request, array $invokeArgs)
    {
        ...
        function _initSetupForm(Am_Form_Setup $form)
    {
        ...
        $form->addSelect('hosted', 'id="hosted-select"', array('options'=>array(0=>'Transparent Redirect', 1=>'Hosted Form', 2=>'Drop In UI')))->setLabel(
            "Mode\n".
            "see below");
...
class Am_Form_CreditCard_Braintree extends Am_Form_CreditCard
{
    ...
    public
        function __construct(Am_Paysystem_CreditCard $plugin, $formType = self::PAYFORM)
    {
        $this->plugin = $plugin;
        ...
        function init()
    {
        ...
        if ($this->plugin->getConfig('hosted')==1)
            $this->createFormHosted();
        else if($this->plugin->getConfig('hosted')==2)
   ...
   function formError(msg){
    if(msg.message) 
    msg = msg.message;
    frm.find('#cc-number').after('<span class="error">'+msg+'</span>');
   }
...
CUT
    ...
    function createFormDropIn(){
            ...
            $this->addStatic('_cc_info', array('class' => 'no-label'))->setContent(<<<CONT
...
jQuery(document).ready(function(){
   ...
   function formError(msg){
    if(msg.message) 
    msg = msg.message;
    frm.find('#dropin-container').after('<span class="error">'+msg+'</span>');
   }
...
//            $this->addClientToken($form);
        ...
        function createUpdateForm()
    {
        ...
        $form->insertBefore(
            (new Am_Form_Element_Html())
                ...
                ->setLabel(___('Current Credit Card Info')), array_shift($elements)
        );
        ...
        function ccAction()
    {
        ...
        if (($this->getParam('http_status') && $this->getParam('hash')) || ($this->plugin->getConfig('hosted') && $this->getParam('nonce')))
        {
            if ($this->processCc())
                return;
        }
        ...
        function updateAction()
    {
        ...
        else if ($this->plugin->getConfig('hosted') && $this->getParam('nonce'))
        {
            $user = $this->getDi()->auth->getUser(true);
            if (!$user)
                throw new Am_Exception_InputError("You are not logged-in");
