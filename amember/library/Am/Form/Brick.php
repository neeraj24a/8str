    ...
    public function insertBrick(HTML_QuickForm2_Container $form)
    {
        ...
        if ($this->getConfig('two_rows') && $this->getConfig('display') != self::DISPLAY_BOTH_SINGLE_INPUT) {
            ...
            $row1 = $form->addGroup('', array('id' => 'name-0'))
                ->setLabel(
                    $this->getConfig('display') == self::DISPLAY_BOTH_REVERSE ?
                        $this->___('Last & First Name') :
                        $this->___('First & Last Name'));
            if (!$this->getConfig('not_required') && !$disabled) {
                $row1->addRule('required');
            }
            $row2 = $row1;
...
class Am_Form_Brick_Agreement extends Am_Form_Brick
{
    protected $labels = array(
        ...
        'I have read and agree to the %s%s%s',
        'Please agree to %s',
    );
    ...
    public function init()
    {
        $di  = Am_Di::getInstance();  $type = $this->getConfig('agreement_type');
        $di->hook->add([Am_Event::SIGNUP_USER_ADDED, Am_Event::SIGNUP_USER_UPDATED], function(Am_Event $e) use ($di, $type){
                $user = $e->getUser();
                $vars = $e->getVars();
                if(!empty($vars['_i_agree'][$type])&&$vars['_i_agree'][$type]){
                    $di->userConsentTable->recordConsent(
                            $user, 
                            $type, 
                            $di->request->getClientIp(), 
                            sprintf("Signup Form: %s", $di->surl($e->getSavedForm()->getUrl())));
                }
    ...
    public function insertBrick(HTML_QuickForm2_Container $form)
    {
        ...
        if(!$this->getConfig('agree_invoice')&& ($user = $di->auth->getUser()) && $di->userConsentTable->hasConsent($user, $type))
            return; // User has it already
        $el_name = "_i_agree[{$type}]";
        
        if(!empty($this->getAgreement()->url)){
            $this->config['do_not_show_agreement_text'] = true;
        }
        ...
        if (!$this->getConfig('do_not_show_agreement_text')) {
            if ($this->getConfig("is_popup")) {
                $form->addEpilog('<div class="agreement" style="display:none" id="'.$this->getConfig('agreement_type').'">' . $this->getText() . '</div>');
            } else {
        ...
        $data = array();
        $header = json_encode($this->getTitle());

        if(!empty($this->getAgreement()->url)){
            $url = $this->getAgreement()->url;
            ...
            $attrs = Am_Di::getInstance()->view->attrs(array(
                'href' => "javascript:;",
                'class' => 'local-link',
                'onclick' => 'jQuery("#'.$this->getConfig('agreement_type').'").amPopup({width:400, title:' . $header . '});'
            ));
            
        }
        

        $label = $this->getConfig('is_popup') ?
                $this->___('I have read and agree to the %s%s%s',
                    "<a {$attrs}>",
                    $this->getTitle(),
                    '</a>') :
                $this->___('I have read and agree to the %s', $this->getTitle());
        if ($this->getConfig('no_label')) {
            $data['content'] = $label;
        }
        $checkbox = $conteiner->addAdvCheckbox($el_name, array(), $data);
        if (!$this->getConfig('no_label')) {
            $checkbox->setLabel($label);
        }
        $checkbox->addRule('required', $this->___('Please agree to %s', $this->getTitle()));
    ...
    public function getAgreement(){
        return Am_Di::getInstance()->agreementTable->getCurrentByType($this->getConfig('agreement_type'));
    }
    ...
    public function initConfigForm(Am_Form $form)
    {
        ...
        if(!Am_Di::getInstance()->agreementTable->getTypeOptions()){
            $gr->addTextarea("_agreement_text", ['rows' => 20, 'class' => 'el-wide']);
            $gr->addHidden('agreement_type');
        }else{
            $gr->addSelect("agreement_type")
                ->loadOptions(array_merge([''=>___('--- Please Select ---')], Am_Di::getInstance()->agreementTable->getTypeOptions()));
