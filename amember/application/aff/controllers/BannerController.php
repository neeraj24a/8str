...
class Am_BannerRenderer_Banner extends Am_BannerRenderer
{
    protected function _getCode()
    {
        ...
        return sprintf('<a href="%s" rel="nofollow"><img src="%s" border=0 alt="%s" %s></a>',
            $this->getUrl(),
            Am_Di::getInstance()->url(array('file/get/path/%s/i/%d', preg_replace('/^\./', '', $upload->getPath()), $this->aff->pk()),
                null,true,2),
...
class Am_BannerRenderer_Lightbox extends Am_BannerRenderer
{
    protected function _getCode()
    {
        ...
        return sprintf('
            <a href="%s" class="am-lightbox" rel="nofollow" rev="%s" title="%s"><img src="%s" border=0 alt="%s"></a>',
            $this->getDi()->url(array('file/get/path/%s/i/%d',preg_replace('/^\./', '', $upload_big->getPath()),$this->aff->pk()),
                null,true,2),
            $this->getUrl(),
            $this->affBanner->title,
            Am_Di::getInstance()->url(array('file/get/path/%s/i/%d',preg_replace('/^\./', '', $upload->getPath()),$this->aff->pk()),
