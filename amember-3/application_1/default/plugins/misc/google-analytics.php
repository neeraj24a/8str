...
class Am_Plugin_GoogleAnalytics extends Am_Plugin
{
    ...
    function onSetupForms(Am_Event_SetupForms $forms)
    {
        ...
        $form->addMagicSelect('analytics_exclude_if_product')
            ->setLabel("Exclude products\nDo not track entire conversion if invoice contains any of these products")
            ->loadOptions($this->getDi()->productTable->getOptions(true));
    }
...
CUT;
        ...
        if (array_intersect($product_ids, $this->getDi()->config->get('analytics_exclude_if_product', array())))
            return $out; // exclude if product option active
