        ...
        foreach ((array) $this->validateFunc as $f) {
            switch ($f) {
                ...
                case 'emails':
                    ...
                    break;
                case 'url':
                    $r = $el->addRule('callback', ___("Please enter a valid URL"), array('Am_Validate', 'url'), $runAt);
                    break;
                case 'ip':
                    $r = $el->addRule('callback', ___("Please enter a valid IP address"), array('Am_Validate', 'ip'), $runAt);
                    break;
                default:
                    if (is_callable($f))
...
class Am_CustomFieldSelect extends Am_CustomField
{
    ...
    {
        $el = parent::addToQF2($container, $attr, $data, $runAt);
        $el->loadOptions($this->isArray ? $this->options : array('' => '') + $this->options);
